...
async function connect(){const {token}=await getStored();if(!token)return;const wsOrigin=apiOrigin.replace(/^http/,"ws");const nextSocket=new WebSocket(`${wsOrigin}/api/extension/ws`);const previousSocket=socket;socket=nextSocket;previousSocket?.close();nextSocket.onopen=()=>nextSocket.send(JSON.stringify({type:"authenticate",token}));nextSocket.onmessage=event=>{const message=JSON.parse(event.data);if(message.type==="capture")void capture(message.task_id,message.source_url)};nextSocket.onclose=()=>{if(socket===nextSocket)setTimeout(()=>{if(socket===nextSocket)void connect()},3000)}}
